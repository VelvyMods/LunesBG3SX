    ...
    if parent then
        ...
        for uuid,origin in pairs(Data.Origins) do
            if Helper.StringContains(uuid, instance.Uuid) then
                foundOrigin = true
                instance.CharacterButton = charGroup:AddImageButton(origin,"EC_Portrait_"..origin, UI.Settings["PartyButtonSize"])
            end
        end
...
function PartyInterface:AddNPC(parent, uuid)
    ...
    local parent = parent or nil
        ...
        for uuid,origin in pairs(Data.Origins) do
            if Helper.StringContains(uuid, instance.Uuid) then
                foundOrigin = true
                instance.CharacterButton = npcGroup:AddImageButton(origin,"EC_Portrait_"..origin, UI.Settings["PartyButtonSize"])
            end
        end
        if not foundOrigin then
